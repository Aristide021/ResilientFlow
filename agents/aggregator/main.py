...
def process_image_endpoint():
    """HTTP endpoint to process satellite image"""
    try:
        ...
        if not bucket_name or not blob_name:
            return jsonify({'error': 'bucket and blob parameters required'}), 400
        
        result = agent.process_satellite_image(bucket_name, blob_name)
        return jsonify(result)
        
    except Exception as e:
        agent.logger.error(f"HTTP endpoint error: {str(e)}")
...
def get_status():
    """Get agent status"""
    try:
        state = agent.firestore_client.read_document('agent_state', agent.agent_name)
        return jsonify({
            'agent': 'data_aggregator',
            'state': state or {},
            'config': agent.config
...
def start_background_monitoring():
    ...
    if agent:
        ...
        # Keep agent alive with periodic status updates
        while True:
            time.sleep(60)
            try:
                agent.pubsub_client.broadcast_agent_status('monitoring')
            except Exception as e:
                agent.logger.error(f"Background monitoring error: {e}")
